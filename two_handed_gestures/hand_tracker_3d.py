...
class HandTracker3D():
    ...
    def __init__(self, palm_model, joint_model, anchors_path,
        ...
        self.interp_joint = tf.lite.Interpreter(model_path=joint_model)
    ...
    def preprocess_img(self, img):
        ...
        return img_pad, img_norm, img_small, pad

    def detect_rotated_joints(self, img):
        img_pad, img_norm, img_small, pad = self.preprocess_img(img)
    ...
    def __call__(self, img):
        ...
        kp_orig = np.r_['-1',(self._pad1(joints[:,:2]) @ Minv.T)[:,:2], joints[:,[2]]/256*img_pad.shape[0]]
        # box_orig = (self._target_box @ Minv.T)[:,:2]
        kp_orig[:,:2] -= pad[::-1]
        # box_orig -= pad[::-1]

        return joints, kp_orig
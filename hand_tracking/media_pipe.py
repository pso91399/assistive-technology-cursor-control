...
def mouse_command(keypoints, closed):
    index_closed = keypoints[8, 1] > keypoints[7, 1] or keypoints[8, 1] > keypoints[6, 1]
    middle_closed = keypoints[12, 1] > keypoints[11, 1] or keypoints[10, 1] > keypoints[9, 1]
    ring_closed = keypoints[16, 1] > keypoints[15, 1] or keypoints[14, 1] > keypoints[13, 1]
    pinky_closed = keypoints[20, 1] > keypoints[19, 1] or keypoints[18, 1] > keypoints[17, 1]
...
while True:
    ...
    cv2.rectangle(frame, (width//2- startWidth - 1, height//2 - startHeight-1), (width//2+ startWidth - 1, height//2 + startHeight -1), (30, 144, 255), 3)
    cv2.rectangle(frame, (width//2- closeWidth - 1, height//2 - closeHeight-1), (width//2+ closeWidth - 1, height//2 + closeHeight -1), (30, 144, 255), 3)
    results = hands.process(img)
    if results.multi_hand_landmarks:
        hand_landmarks = results.multi_hand_landmarks[0]
        keypoints = hand_keypoints(hand_landmarks)
        center, radius = palm_center(keypoints)
        message , history = process_pipeline(center, width//2, height//2, history, startWidth, startHeight,closeWidth, closeHeight )
    ...
    if cursor_control == "joystick":
        cv2.circle(frame, tuple(joystick_center), joystick_radius, (255, 0, 0), 2)
